...
@section('content')
    ...
    <ul class="nav" id="spy-pos-fixed">
    	<li><a href="#top"><span class ="glyphicon glyphicon-chevron-up"></span></a></li>
        @foreach($featureGroups as $featureGroup)
        	<li><a href="#{{$featureGroup->groupName}}">{{$featureGroup->groupName}}</a></li>
        @endforeach
    </ul>
	...
	<div class="well well-sm">
		<div class="row">
			<div class="col-md-12">
				<h2 id="top">{{$client->siteName}}<small>&nbsp;Site ID:{{$client->clientID}} | Launch:{{$client->launchDate}}</small></h2></div>
		...
		<div class="panel-body">
			@foreach ($featureGroups as $featureGroup)
				...
				{{Form::open(array('action' => 'ClientFeatureController@clientFeatureEdit'))}}<!--pointing to controller -->
					<div class="row">
						<div class="col-md-10"><h3 id="{{$featureGroup->groupName}}"class="text-muted text-capitalize">{{$featureGroup->groupName}}</h3></div>
